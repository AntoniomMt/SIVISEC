...
min_area_percent = 0.012
tighten_ratio = 0.05  # ← Ceñir cajas
...
while cap.isOpened():
    ...
    # Redimensionamos internamente para procesar
    ...
    for result in results:
        ...
        for box, score, class_id in zip(boxes, scores, class_ids):
            if class_id == 0 and score > conf_threshold:
                ...
                width = x2 - x1
                height = y2 - y1
                x1 += int(width * tighten_ratio)
                x2 -= int(width * tighten_ratio)
                y1 += int(height * tighten_ratio * 0.5)
                y2 -= int(height * tighten_ratio)
    ...
    # Anti-cajas grandes que engloban otras
    filtered_detections = []
