...
min_area = 10000
confidence_threshold = 0.4  # <<<<< NUEVO
...
while True:
    ...
    if frame_count % DETECTION_INTERVAL == 0:
        ...
        for box in results.boxes.data:
            ...
            if int(cls) == 0 and conf >= confidence_threshold:  # <<<<< SOLO SI supera confianza
                x1 = int(x1 * scale_x)
                y1 = int(y1 * scale_y)
                x2 = int(x2 * scale_x)
                y2 = int(y2 * scale_y)
                ...
                # ---- NUEVA ZONA DE EXCLUSIÓN (parte inferior izquierda del frame) ----
                if x1 < 200 and y2 > frame.shape[0] - 100:
                    continue  # ignoramos esta zona problemática
