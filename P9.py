...
# Cargar modelo YOLOv5 Nano (ligero para CPU)
model = YOLO("yolov5n.pt")

# Inicializar la cámara en alta calidad
cap = cv2.VideoCapture(0)
cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1280)
cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 720)

# Resolución reducida para detección rápida
...
while True:
    ...
    # Escalas para dibujar en el frame original
    scale_x = frame.shape[1] / detection_width
    scale_y = frame.shape[0] / detection_height

    person_count = 0  # Contador de personas por frame
    ...
    cv2.putText(frame, f"Personas detectadas: {person_count}", (20, 45),
