...
def detectar_personas(frame):
    ...
    for p_box in personas:
        ...
        data_ant = personas_memoria.get(persona_id, {"estado":"normal","ha_escondido":False,"tiempo_sospechoso":0,"frames_estado":0})
        ...
        sostenida = any(colision(mano, p_box) and any(colision(mano, botella) for botella in botellas) for mano in mano_boxes)
        nuevo_estado = estado
        ...
        nuevas_memorias[persona_id] = {"estado":nuevo_estado,"centro":(px, py),"ha_escondido":ha_escondido,"tiempo_sospechoso":tiempo_sospechoso,"frames_estado":frames_estado}
    ...
    for pid, data in personas_memoria.items():
        ...
        if estado=="normal": color,label=(0,255,0),"Persona"
        elif estado=="sosteniendo": color,label=(0,255,255),"Sosteniendo mercancia"
        elif estado=="escondiendo": color,label=(0,165,255),"Escondiendo mercancia"
        elif estado=="posible_robo": color,label=(0,0,255),"Posible robo"
        else: color,label=(128,200,255),"Comportamiento sospechoso"

        cv2.rectangle(frame,(p_box[0],p_box[1]),(p_box[2],p_box[3]),color,2)
        cv2.putText(frame,f"{label} (ID {pid})",(p_box[0],p_box[1]-10),cv2.FONT_HERSHEY_SIMPLEX,0.8,color,2)

    return frame, []  # devolvemos frame procesado y lista de alertas vacía
