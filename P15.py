...
# Captura de cámara y resolución 1280x720
cap = cv2.VideoCapture(0)
cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1280)
cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 720)

# Resolución para detección (más baja para rendimiento)
detection_width, detection_height = 416, 234
frame_count = 0
DETECTION_INTERVAL = 3
min_area = 10000  # Ignorar cosas pequeñas
...
while True:
    ...
    # Filtrar tracks activos y ordenarlos por ID
    ...
    for display_id, track in enumerate(active_tracks, start=1):
        ...
        # Intentar debajo de la caja
        tag_x = x1
        tag_y = y2
        if tag_y + tag_height > frame_h:
            # Si no cabe abajo, intenta arriba
            tag_y = y1 - tag_height
            if tag_y < 0:
                # Si no cabe arriba, muévela a la izquierda
                tag_y = y1
                tag_x = x1 - tag_width
                if tag_x < 0:
                    # Si ni eso, reduce el tamaño
                    tag_x = 0
                    tag_width = 100

        # Dibuja fondo y texto
